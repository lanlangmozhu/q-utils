[**qh5-utils v1.0.0**](../../../README.md)

***

[qh5-utils](../../../README.md) / network

# network

网络操作相关工具函数

## Functions

- [downloadFile](functions/downloadFile.md)
- [fetchWithTimeout](functions/fetchWithTimeout.md)
- [jsonp](functions/jsonp.md)
- [ping](functions/ping.md)
- [request](functions/request.md)
- [parseURLParams](functions/parseURLParams.md)
- [buildURL](functions/buildURL.md)
- [getURLPath](functions/getURLPath.md)
- [getURLDomain](functions/getURLDomain.md)
- [isHTTPS](functions/isHTTPS.md)
- [getFileNameFromURL](functions/getFileNameFromURL.md)
- [isAbsoluteURL](functions/isAbsoluteURL.md)
- [joinURL](functions/joinURL.md)
